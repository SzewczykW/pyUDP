...
class UDP:
    ...
    def stop(self) -> None:
        ...
        `self.set_start()`.
        """
        self.logger.debug("Stopping UDP communication...")
        ...
        self.logger.debug("UDP stopped communication.")

    def start(self) -> None:
        """
        Start the UDP communication by creating RX and TX threads.
        """
        self.logger.debug("Starting UDP communication...")
        self.__stop.clear()

        self._rx = UDP._RX(self.logger, self._socket, self.__stop, self.rx_pkt_size)
        self._tx = UDP._TX(self.logger, self._socket, self.__stop)

        self._rx.start()
        self._tx.start()

        self.logger.debug("UDP started communication.")
    ...
    def recv(self, n: int = 1) -> Optional[Tuple[bytes, ...]]:
        ...
        * (Optional[Tuple[bytes, ...]]): Yielded received packet poped from RX queue.
        ...
        if self.__stop.is_set():
            raise UDPCommunicationIsStopped("Cannot perform UDP action when threads are not running!")

        for _ in range(n):
            # Do we need to yield n packets? What is use-case for this?
            yield self._rx.get_pkt()

    def sendto(self, pkt: Tuple[Optional[bytes], str]) -> None:
        """
    ...
    class _RX(threading.Thread):
        ...
        def run(self) -> None:
            ...
            while True:
                print(self.__stop)
                if self.__stop.is_set():
                    self.logger.debug("Stopping UDP RX communication...")
                    break

                try:
                    data = self._socket.recvfrom(self.pkt_size)
                    self._rx_queue.put(data)
                    self.logger.debug(
                        f"Received message from {self._socket.getsockname()}: {data = }"
                    )
                except OSError as e:
                    self.logger.debug(f"Socket error in RX: {e}")
                    continue
                except Full:
    ...
    class _TX(threading.Thread):
        ...
        def run(self) -> None:
            ...
            while True:
                print(self.__stop)
                if self.__stop.is_set():
                    self.logger.debug("Stopping UDP TX communication...")
                    break

                try:
                    data, remote_address = self._tx_queue.get()
                    ret = self._socket.sendto(data, remote_address)
                    if ret != len(data):
                        self.logger.warning(
                            f"Could not send message in full. Message: {data}"
                        )
                    self.logger.debug(
                        f"Sent message to {remote_address} containing: {data}"
                    )
                except OSError as e:
                    self.logger.debug(f"Socker error in TX: {e}")
                    continue
                except Empty:
                    self.logger.debug("TX queue is empty, nothing to send")
                    continue
                except ValueError as e:
                    self.logger.error(
                        f"Invalid structure in TX queue. Must be (data, remote_address). Dumping last added element"
                    )
                    _ = self._tx_queue.get()  # Dump invalid element
                    continue
