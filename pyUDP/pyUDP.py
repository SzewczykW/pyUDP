...
class UDP:
    ...
    def __init__(
        ...
        * `rx_pkt_size` (int): Maximum packet size to be received.
        ...
        self._socket = socket.socket(socket.AF_INET, socket.SOCK_DGRAM | socket.SOCK_NONBLOCK)
    ...
    class _RX(threading.Thread):
        ...
        def run(self) -> None:
            ...
            while not self.__stop():
                try:
                    response, remote_address = self._socket.recvfrom(self.pkt_size)
                    if response:
                        self._rx_queue.put_nowait((response, remote_address))
                        self.logger.debug(
                            f"Received message from {self._socket.getsockname()}:{remote_address}"
                        )
                except OSError as e:
                    continue
                except Full:
        ...
        def run(self) -> None:
            ...
            while not self.__stop():
                try:
                    ...
                    self.logger.debug(
                        f"Sent message to {remote_address}"
                    )
                except OSError as e:
                    continue
                except Empty:
